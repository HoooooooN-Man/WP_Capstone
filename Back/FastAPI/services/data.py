...
def get_market_regime(model_version: str = "latest") -> dict:
    """
    Tier A 종목 비율로 현재 시장 국면(greed / neutral / fear)을 판단한다.
    ...
    def fetch() -> dict:
        ...
        latest_date: str | None = date_row[0] if date_row else None
        if latest_date is None:
            raise RuntimeError("scores 테이블에 데이터가 없습니다.")
        ...
        total_count: int = int(agg[0]) if agg else 0
        tier_a_count: int = int(agg[1]) if agg else 0
        tier_a_ratio: float = round(tier_a_count / total_count * 100, 2) if total_count else 0.0

        # 국면 판정
        if tier_a_ratio >= 10.0:
            status, weather = "greed", "맑음"
            message = "상승 모멘텀이 강합니다. 적극적인 투자가 유리한 국면입니다."
        elif tier_a_ratio >= 5.0:
            status, weather = "neutral", "흐림"
            message = "방향성 탐색 구간입니다. 선별적인 종목 접근이 필요합니다."
        else:
            status, weather = "fear", "비"
            message = "시장 변동성이 커지고 있습니다. 현금 비중 확대를 권장합니다."
...
def get_kospi200_portfolio(
    ...
    """
    KOSPI 200 종목 중 사용자 성향에 맞는 Top 10 포트폴리오를 반환한다.

    - ``growth`` : AI score 기준 상위 10개
    - ``stable`` : Tier A·B + PBR < 1.5 조건 충족 상위 10개
    """
    ver = _resolve_version(model_version)

    from ..core.config import SEED_CSV

    # seed.csv 경로를 SQL 문자열 리터럴로 안전하게 삽입
    # (사용자 입력이 아닌 config 상수이므로 경로 삽입은 안전)
    seed_path = str(SEED_CSV).replace("\\", "/").replace("'", "''")
    ...
    def fetch() -> list[dict]:
        ...
        if portfolio_type == "stable":
            # 안정형: Tier A/B + 최신 분기 PBR < 1.5
            sql = f"""
                ...
                FROM scores s
                INNER JOIN read_csv_auto('{seed_path}') k
                ...
                LIMIT 10
            """
            rows = con.execute(sql, [ver, latest_date]).fetchdf()
        else:
            # 성장형: AI score 기준 상위 10개
            sql = f"""
                SELECT
                    s.ticker,
                    s.name,
