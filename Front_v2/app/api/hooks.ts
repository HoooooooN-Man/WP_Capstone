...
/** 종목별 매칭 뉴스 — title + report 본문 경계단어 매칭. */
export function useStockNews(ticker: string, limit: number = 8) {
  return useQuery({
    queryKey: ['stock_news', ticker, limit],
    queryFn: () =>
      api.get<{
        ticker: string; name?: string;
        items: Array<{
          news_id: string; title: string; source?: string;
          published_at?: string; url?: string;
          category?: string; category_label?: string;
          score?: number; match_via?: 'title' | 'report_body';
        }>;
      }>(`/api/v1/stocks/${ticker}/news`, { limit }),
    enabled: !!ticker,
    staleTime: 60_000,
  });
}

/** webnews 6 카테고리 LLM 요약 (최신 일자). */
export function useNewsCategories(date?: string) {
  return useQuery({
    queryKey: ['news_categories', date],
    queryFn: () =>
      api.get<{
        date: string | null;
        categories: Array<{
          ...
          main_themes?: Array<{ theme: string; explanation?: string; market_relevance?: string }>;
          sector_signals?: Array<{ sector: string; signal: string; reason?: string }>;
          watch_points?: string[];
          item_count: number; generated_at?: string;
        }>;
      }>('/api/v1/news/categories', date ? { date } : {}),
    staleTime: 300_000,
  });
}

/** 카테고리별 item 목록. */
export function useCategoryItems(category: string, top_n: number = 20) {
  return useQuery({
    queryKey: ['category_items', category, top_n],
    queryFn: () =>
      api.get<{
      ...
      }>(`/api/v1/news/category/${category}/items`, { top_n }),
    enabled: !!category,
    staleTime: 60_000,
  });
}
