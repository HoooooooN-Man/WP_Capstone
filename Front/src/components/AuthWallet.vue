<template>
  <div class="auth-wallet min-h-screen w-full flex items-center justify-center bg-[#050505] relative overflow-hidden font-sans">
    ...
    >
      <div class="absolute inset-0 rounded-r-[2.5rem] rounded-l-none opacity-70 pointer-events-none mix-blend-overlay bg-[url('https://www.transparenttextures.com/patterns/leather.png')]"></div>
      
      <div class="absolute left-0 top-0 bottom-0 w-8 bg-gradient-to-r from-black/20 via-white/5 to-transparent rounded-l-none pointer-events-none z-10"></div>
      <div class="absolute left-0 top-0 bottom-0 w-px bg-black/30 pointer-events-none"></div>

      <div class="stitch-lines absolute inset-0 rounded-r-[2.5rem] rounded-l-none pointer-events-none z-0"></div>

      <div class="relative z-20 text-center pl-8">
      ...
      <form @submit.prevent class="relative z-20 space-y-3.5 w-full max-w-[320px] pl-8">
        <input v-model="email" type="email" placeholder="EMAIL ADDRESS" required class="w-full px-6 py-4.5 bg-black/40 border border-white/5 rounded-2xl text-xs text-white placeholder:text-[#5c4033] focus:border-[#d9b9a9]/50 focus:bg-black/60 outline-none transition-all tracking-widest shadow-inner" />
        <input v-model="password" type="password" placeholder="PASSWORD" required class="w-full px-6 py-4.5 bg-black/40 border border-white/5 rounded-2xl text-xs text-white placeholder:text-[#5c4033] focus:border-[#d9b9a9]/50 focus:bg-black/60 outline-none transition-all tracking-widest shadow-inner" />
      </form>

      <div class="relative z-20 w-full text-center pb-2 pl-8">
        <p class="text-[9px] text-[#c9a999] uppercase tracking-widest mb-3 italic opacity-60">or continue with:</p>
        <div class="flex justify-center gap-4">
          <button type="button" class="w-10 h-10 rounded-full bg-white flex items-center justify-center shadow-lg hover:scale-110 transition-transform border border-white/10"><img src="https://www.gstatic.com/images/branding/product/2x/googleg_48dp.png" class="w-5 h-5" /></button>
...
@keyframes walletDrop {
  ...
  100% { 
    transform: perspective(1200px) translateY(82vh) rotateX(15deg); 
    filter: brightness(0.7) drop-shadow(0 20px 50px rgba(0,0,0,1));
  }
}
